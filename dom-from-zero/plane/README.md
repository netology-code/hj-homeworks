...
В схеме расположения мест каждый элемент массива представляет количество мест в текущем ряду.
...
Кнопка с идентификатором `btnSetFull` позволяет обозначить все места в самолете как занятые. Пока не отображена схема самолёта, эта кнопка должна быть заблокирована.
...
Ряд седений в самолёте должен иметь следующую структуру:
```html
...
В тег `h2` так же необходимо поместить номер ряда. Номерация начинается с `1`.
...
Для того чтобы обозначить сиденье занятым в тег `div.seat` необходимо добавить класс `adult` для мест с полной стоимостью, и `half` для мест за половину цены.
...
При любых изменениях состояние мест необходимо обновлять количество занятых мест, занятых мест за полную стоимость и занятых мест за половину цены.
